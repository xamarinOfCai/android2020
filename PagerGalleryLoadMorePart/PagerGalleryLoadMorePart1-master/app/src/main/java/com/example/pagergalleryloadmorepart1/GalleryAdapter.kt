...
class GalleryAdapter(var galleryViewModel: GalleryViewModel) : ListAdapter<PhotoItem, MyViewHolder>(DIFFCALLBACK) {
    companion object {
        const val NORMAL_VIEW_TYPE = 0
        const val FOOTER_VIEW_TYPE = 1
    }

    var footerViewStatus = DataStatus.DATA_STATUS_CAN_LOAD_MORE
            ...
            holder = MyViewHolder(
                LayoutInflater.from(parent.context).inflate(
                    ...
                    it.setOnClickListener{itemview->
                        //progressbar显示出来，正在加载
                        itemview.progressBar.visibility = View.VISIBLE
                        itemview.textView.text="正在加载，请稍后"
        ...
        if (position == itemCount - 1) {
            with(holder.itemView){
                when(footerViewStatus){
                    DataStatus.DATA_STATUS_NETWORK_ERROR->{
                        progressBar.visibility = View.GONE
                        textView.text = "网络故障，请稍后再试"
                        this.isClickable = true
                    }
                    DataStatus.DATA_STATUS_CAN_LOAD_MORE->{
                        progressBar.visibility = View.VISIBLE
                        textView.text = "正在加载"
                        this.isClickable = false
                    }
                    DataStatus.DATA_STATUS_NO_MORE->{
                        progressBar.visibility = View.GONE
